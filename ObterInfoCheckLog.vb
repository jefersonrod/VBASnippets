...
' s is the variable to process
    ...
    If (InStr(s, "/") > 0) Then
        pos = InStr(s, "/2019")
        data = Mid((s), 1, pos + 4)
        cdata = CDate(Format(data, "dd/mm/yyyy"))
        'cdata = CDate(data)
        'cdata = DateValue("dd/mm/yyyy")
        'MsgBox (CStr(cdata))
        'data = CStr(cdata)
        pos = InStr(s, ":")
        hora = Mid(s, pos - 2, 5)
    ...
    If ((s = chin) Or (s = chinast)) Then
        GoTo processarCheckIN:
    End If
    If ((s = chout) Or (s = choutast)) Then
...
If (cdata <> "00:00:00") Then
    Worksheets(plan).Cells(linha, colData) = cdata
Else
...
' s is the variable to process
    ...
    If (Recebeucontato) Then
        Worksheets(plan).Cells(linha, colCINTecL8colE) = fnConverterUTF8.fnConverterUTF8(LTrim(s))
        'Debug.Print s
        Recebeucontato = False
    End If
    ...
    If (Recebeuorienta) Then
        Worksheets(plan).Cells(linha, colCINTecL10colF) = fnConverterUTF8.fnConverterUTF8(LTrim(s))
        'Debug.Print s
    ...
    If (realizoumig) Then
        Worksheets(plan).Cells(linha, colCINTecL12colG) = fnConverterUTF8.fnConverterUTF8(LTrim(s))
        'Debug.Print s
        realizoumig = False
    End If
    ...
    If (PossuiWhatsapp) Then
        Worksheets(plan).Cells(linha, colCINTecL14colH) = fnConverterUTF8.fnConverterUTF8(LTrim(s))
        'Debug.Print s
    ...
    If (Informarsobreolink) Then
        Worksheets(plan).Cells(linha, colCINTecL16colI) = fnConverterUTF8.fnConverterUTF8(LTrim(s))
        'Debug.Print s
    ...
    If (Enviofotos) Then
        Worksheets(plan).Cells(linha, colCINTecL18colJ) = fnConverterUTF8.fnConverterUTF8(LTrim(s))
        'Debug.Print s
        Enviofotos = False
    End If
    ...
    If (sobreacompanhamento) Then
        Worksheets(plan).Cells(linha, colCINResL22colK) = fnConverterUTF8.fnConverterUTF8(LTrim(s))
        'Debug.Print s
        sobreacompanhamento = False
    End If
    If (InStr(s, "sobre acompanhamento") > 0) Then
        sobreacompanhamento = True
    End If
    'colL Tem algum chamado aberto?
    If (algumchamado) Then
        Worksheets(plan).Cells(linha, colCINResL24colL) = fnConverterUTF8.fnConverterUTF8(LTrim(s))
        'Debug.Print s
        algumchamado = False
    End If
    If (InStr(s, "algum chamado") > 0) Then
        algumchamado = True
    End If
    ...
    If (assinarOS) Then
        ...
        assinarOS = False
    End If
    If (InStr(s, "assinar OS somente") > 0) Then
        assinarOS = True
    End If
...
' s is the variable to process
    ...
    If (deuorienta) Then
        Worksheets(plan).Cells(linha, colCOUTTecL8colP) = fnConverterUTF8.fnConverterUTF8(LTrim(s))
        'Debug.Print s
        deuorienta = False
    End If
    ...
    If (momentodamig) Then
        Worksheets(plan).Cells(linha, colCOUTResL12colQ) = fnConverterUTF8.fnConverterUTF8(LTrim(s))
        'Debug.Print s
        momentodamig = False
    End If
    ...
    If (avaliacao) Then
        Worksheets(plan).Cells(linha, colCOUTResL14colR) = fnConverterUTF8.fnConverterUTF8(LTrim(s))
        'Debug.Print s
        avaliacao = False
    End If
    ...
    If (houvermaisitens) Then
        Worksheets(plan).Cells(linha, colCOUTResL16colS) = fnConverterUTF8.fnConverterUTF8(LTrim(s))
        'Debug.Print s
